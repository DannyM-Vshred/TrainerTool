...
describe('Purchase a single Bottle Supplement Only', () => {
    before(() => {
        ...
        cy.fixture('supplements/sixBottleSupplements').as('sixBottle')
    })
    ...
    it.only('can purchase monthly supplement subscriptions', function() {
        ...
        cy.get(supplement).each((sixBottleSupp)=>{
            ...
            const lName = sixBottleSupp.lname + dateS
            ...
            cy.get('td.py-1').should('contain.text', sixBottleSupp.orderItem1)
                .next().should('contain.text', sixBottleSupp.orderQty)
            ...
            //complete profile
            cy.get('#profile-gender').select(sixBottleSupp.gender)
           ...
           //  cy.contains('#tab-subscriptions td', 'bt_turmeric-black-monthly')
            ...
            cy.contains('#orders-table td.th-name', sixBottleSupp.confirmOrder1).should('exist')
